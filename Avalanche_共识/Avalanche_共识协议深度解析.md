...
2. **阈值过滤放大差异**：需要 $\geq \alpha > k/2$ 才算成功，多数派更容易达到阈值
...
- 可能有 $\geq \alpha$ 票支持 $v=0$
- 同时有 $\geq \alpha$ 票支持 $v=1$
...
一个原本偏好 $v=0$ 的节点，如果它采样到 $\geq \alpha$ 个偏好 $v=1$ 的节点，就会切换偏好到 $v=1$ 。反之亦然。
...
这是一个偏好 $v=0$ 的节点切换到 $v=1$ 的概率（采样到 $\geq \alpha$ 个 $v=1$ ）。

$$P_0(p) = P(X \leq k - \alpha) = \sum_{i=0}^{k-\alpha} \binom{k}{i} p^i (1-p)^{k-i}$$

这是一个偏好 $v=1$ 的节点切换到 $v=0$ 的概率（采样到 $\geq \alpha$ 个 $v=0$ ，等价于 $\leq k-\alpha$ 个 $v=1$ ）。

**第三步：对称性证明 $P_0(p) = P_1(1-p)$**

**简洁证明**：

$P_0(p)$ 问的是：在 $v=1$ 比例为 $p$ 的网络中，采样到 $\geq \alpha$ 个 $v=0$ 的概率。

$P_1(1-p)$ 问的是：在 $v=1$ 比例为 $(1-p)$ 的网络中，采样到 $\geq \alpha$ 个 $v=1$ 的概率。
...
| 采样目标 | $\geq \alpha$ 个 $v=0$ | $\geq \alpha$ 个 $v=1$ |
...
节点 $B$ 期望获得 16 票支持 $v=1$ ，仍然 $\geq \alpha = 15$ 。

**节点 B 接受 $v=0$ 的概率**：

要让 $B$ 接受 $v=0$ ，需要在连续 20 轮中，每轮采样到 $\geq 15$ 个 $v=0$ 。
