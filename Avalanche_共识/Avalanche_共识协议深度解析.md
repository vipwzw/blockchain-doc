...
- 多数派（60%）被采样成功切换的概率：$P_0(0.6) = P_1(0.4)$ ≈ 0.16%
- 少数派（40%）被采样成功切换的概率：$P_1(0.6)$ ≈ 9.9%
...
**引理**：存在临界比例 $p^{\ast}$ ，当网络中偏好 $v=1$ 的比例超过 $p^{\ast}$ 后，系统最终收敛到 $v=0$ 的概率可以忽略不计。
...
这证明了跨过临界点（约 $p^{\ast} \approx 0.6$ ）后，回退概率小到可以完全忽略。 $\square$
...
- 来自诚实节点的 $v=1$ 票：20 × 80% = 16 票
- 来自拜占庭节点的 $v=1$ 票：20 × 20% = 4 票
- 总计：约 20 票支持 $v=1$
